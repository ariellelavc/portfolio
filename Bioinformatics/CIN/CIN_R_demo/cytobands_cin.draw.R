...
#	dev.new() width=12, height=15
 # dev=png(file=paste(title_text, '.png',sep=''), width=720, height=720, pointsize=9)
#	dev=pdf(file=paste(title_text, '.pdf',sep=''), width=12, height=15, title=title_text, pointsize=9)
	...
	title(main=paste('chr',chr,'cytobands CIN'),cex=4)
	#title(sub=paste('chromosome',chr,'cytobands CIN overview'),cex.sub=1.25)

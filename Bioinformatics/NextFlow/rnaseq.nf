...
process getSRAIDs {
	...
	script:
	"""
	esearch -db sra -query $projectID  | efetch --format runinfo | grep SRR | cut -d ',' -f 1 | head -6 > sra.txt
	"""
}
...
process star {
	...
	"""
	STAR --genomeDir $index --limitGenomeGenerateRAM 12000000000 --readFilesIn $read --runThreadN ${task.cpus} --readFilesCommand zcat --outSAMtype BAM Unsorted --outFileNamePrefix $readName
	"""
}
